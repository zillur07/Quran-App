...
import '../models/expanded_model.dart';
...
class ApiController extends GetxController {
  ...
  Future<void> getAllSuraFromLocal() async {
    var res = await GetStorage().read('all') as List<Result>;
    allverse.addAll(res);
    log('$allverse' '>>>>>>>>>>>>>>>>>>>>>>>>');
  }
  ...
  getAllSura() async {
    ...
    try {
      ...
      if (allSura.value!.search!.results!.isNotEmpty) {
        ...
        log('${allverse[0]}' 'mmmmmmmmmmmmmmmm');
      } else {
        print('has no data');
